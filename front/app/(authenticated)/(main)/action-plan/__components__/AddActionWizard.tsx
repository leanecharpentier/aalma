...
export default function AddActionWizard({
  onClose,
  catalogActions,
  participants,
}: AddActionWizardProps) {
    ...
    <div className="flex flex-1 flex-col bg-gray-40 rounded-xl shadow-card-light p-5 overflow-y-auto min-w-0 min-h-0 h-full">
      ...
      {currentStep === 1 && (
        <StepChooseAction
          ...
          favorites={catalogActions.favorites}
          discover={catalogActions.discover}
        />
      )}
      {currentStep === 2 && <StepPlanify onNext={() => setCurrentStep(3)} />}
      {currentStep === 3 && (
        <StepParticipants onSave={onClose} participants={participants} />
      )}
    </div>
  );
}
...
function ActionPickCard({
  ...
  return (
    ...
    >
      ...
      {/* Category badge */}
      <span className="w-fit inline-flex items-center border border-gray-500 rounded-full px-2 py-1 text-xs font-bold text-gray-500">
        {category}
      </span>
